...
<template>
  <div class="no-fixed-width-container">
    ...
    <el-form ref="form" :model="this" :rules="formRules">
      ...
      <el-form-item v-if="inAction" class="input-row last" label="GA Code:" prop="ga">
...
export default {
    ...
    formRules() {
      return {
        ...
        emailCode: [
          ...
          { pattern: /^\d{6}$/, trigger: "blur", message: "Email code must be a six-digit number" },
        ],
      };
    },
  ...
  methods: {
    ...
    async onApproveClick() {
      ...
      const params = await this.$store.dispatch("getTransactionParams", this.tx_id);
      let result;
      try {
        this.ledgerConnecting = true;
        const sdk = await LedgerSDK.create(this.workflowDetail.coin_type);
        result = await sdk.signTransaction(params);
      } finally {
      ...
      await this.$store.dispatch("signTransaction", {
        ...
        data: {
          audit_result: true,
          tx_body: result.transaction,
          tx_signatures: result.signatures,
        },
        ga: this.ga,
        emailCode: this.emailCode,
      });
    ...
    onActionClick() {
      this.$router.replace({ name: "withdraw-operation-action", params: { tx_id: this.tx_id } });
    },
  },
};
</script>
